    ...
    -->
			<xsl:if test="//_get/equipment_id">
				...
				<input
						...
						value="{//equipment_get_by_id/equipment_type_id}" />
				<input
						type="hidden"
			...
			<table>
				<tr>
					...
					<td>
						<xsl:if test="//_get/equipment_id">
							<xsl:value-of
									select="//equipment_get_by_id/equipment_number" />
						</xsl:if>
						<xsl:if test="not(//_get/equipment_id)">
							<input
									type="text"
					...
					<td rowspan="6">
						<xsl:if test="//_get/equipment_id">
							<img
									src="{//equipment_get_by_id/image_pointer}" />
						</xsl:if>
					</td>
				</tr>
				<tr>
            ...
            <!-- Existing equipment type cannot be changed -->
					<xsl:if test="//_get/equipment_id">
						<td>
							<xsl:value-of
									select=" //equipment_types_get_all[ equipment_type_id= //equipment_get_by_id/equipment_type_id ]/name" />
						</td>
					</xsl:if>
            ...
            <!-- Choose equipment type. -->
							...
							<select name="equipment_type_id">
								<xsl:for-each select="//equipment_types_get_all">
									<option value="{equipment_type_id}">
										<xsl:if
												test=" equipment_type_id= //equipment_get_by_id/equipment_type_id ">
				...
				<tr>
					...
					<td>
						<xsl:if test="//_get/equipment_id">
							<xsl:value-of
									select=" //locations_get_all[ location_id=//equipment_get_by_id/location_id ]/name" />
						</xsl:if>
						<xsl:if test="not(//_get/equipment_id)">
							<select name="location_id">
								<xsl:for-each select="//locations_get_all">
									<option value="{location_id}">
										<xsl:if
												test=" location_id= //equipment_get_by_id/location_id ">
											<xsl:attribute name="selected">
                                    selected
                                </xsl:attribute>
										</xsl:if>
